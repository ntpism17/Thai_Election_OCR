...
class RAGPipeline:
    ...
    def _build_faiss(self, texts: List[str]):
        ...
        with open(CACHE_PATH, "wb") as f:
            pickle.dump({"model": EMBED_MODEL_NAME, "num_chunks": len(
                texts), "embeddings": embeddings}, f)
    ...
    def _rrf(self, ranked_lists: List[List[int]], k: int = RRF_K) -> List[int]:
        ...
        for ranked in ranked_lists:
            for rank, doc_idx in enumerate(ranked):
                scores[doc_idx] = scores.get(
                    doc_idx, 0.0) + 1.0 / (k + rank + 1)
    ...
    def retrieve(self, query: str, top_k: int = TOP_K_CONTEXT) -> List[Dict]:
        if not self.chunks or self.bm25_index is None or self.faiss_index is None:
            return []
